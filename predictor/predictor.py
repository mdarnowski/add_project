...
channel.basic_consume(queue='trained_model', on_message_callback=load_model_weights, auto_ack=True)
...
def callback(ch, method, properties, body):
    ...
    response = json.dumps({'image_path': message['image_path'], 'prediction': int(prediction)})
    channel.basic_publish(exchange='', routing_key='prediction_response_queue', body=response)
