...
def train_model(ch, method, properties, body) -> None:
    ...
    if message.get('train'):
        ...
        weights_id = weights_collection.insert_one({'weights': [w.tolist() for w in model.get_weights()]}).inserted_id
        channel.basic_publish(exchange='', routing_key='trained_model',
...
channel.basic_consume(queue='train_request', on_message_callback=train_model, auto_ack=True)
